...
> ⚠️ **DB safety**: `DATABASE_URL` is the live production DB. **Never run `prisma migrate dev`, `migrate reset`, `db push`, or `db execute`** against it — use `prisma migrate deploy` only. `BlogPostChunk` (pgvector RAG table) is created by raw SQL in a migration and is **NOT** modeled in `schema.prisma`, so `db push` will DROP it. See `docs/db/` / memory for known schema drift.
>
> 🛑 **This is now enforced, not just documented.** `prisma.config.ts` inspects `process.argv` at config-load time — which every Prisma CLI subcommand triggers — and aborts destructive commands before any connection is opened. Wrapping them in npm scripts would not work: the dangerous form *is* `npx prisma db push`, typed directly. To run one deliberately: `ALLOW_DESTRUCTIVE_DB=1 npx prisma <cmd>` (prints a loud warning with the target host).

> 💾 **Backups: this project's Supabase plan (Free) has no project backups** — `scripts/db-backup.mjs` is the only recovery path. `prisma migrate deploy` therefore **aborts unless a verified backup from the last 24h exists** in `backups/` (override: `SKIP_BACKUP_CHECK=1`). Take one with `npm run db:backup`, or use `npm run db:deploy` to back up and deploy in one step. The dump is **data-only** (schema comes from `prisma/migrations/`) and skips regenerable embedding tables. Restore procedure and its limits: [`docs/db/restore.md`](docs/db/restore.md).
