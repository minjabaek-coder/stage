  ...
  return (
    <div className="mx-auto w-full max-w-sm">
      ...
      <p className="mt-2 text-sm text-[#444748]">
        {isSignup
          ? "STAGE 회원으로 39호 매거진과 AI 마에스트로를 만나보세요."
          : "다시 오신 것을 환영합니다."}
      </p>
      ...
      <form action={formAction} className="mt-8 space-y-4">
        ...
        <button
          type="submit"
          disabled={pending}
          className="w-full bg-[#1c1b1b] py-3 font-label text-[11px] font-bold uppercase tracking-widest text-white transition-colors hover:bg-[#6f5c24] disabled:opacity-50"
        >
          {pending ? "처리 중..." : isSignup ? "가입하기" : "로그인"}
        </button>
      </form>

      <p className="mt-6 text-center text-sm text-[#444748]">
        {isSignup ? "이미 계정이 있으신가요? " : "계정이 없으신가요? "}
        <Link
          href={isSignup ? "/auth/login" : "/auth/signup"}
          className="font-medium text-[#6f5c24] hover:underline"
        >
          {isSignup ? "로그인" : "회원가입"}
        </Link>
      </p>

      <p className="mt-8 text-center font-label text-[10px] uppercase tracking-wider opacity-40">
