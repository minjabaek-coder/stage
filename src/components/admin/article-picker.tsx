...
export function ArticlePicker({
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      {open && (
        <div className="absolute bottom-full z-50 mb-1 w-80 rounded-lg border bg-popover p-2 shadow-lg">
          <input
            autoFocus
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="제목·저자 검색"
            className="mb-2 h-8 w-full rounded-md border bg-transparent px-2 text-xs"
          />
